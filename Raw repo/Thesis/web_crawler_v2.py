...
for x in tqdm(rfc_list):
    ...
    while retry:
        try:
            ...
            if names[-1] == "Unknown" or names[-1] == "Yes":
                rfc_list[x]["author"] = names[:-1]
            else:
                rfc_list[x]["author"] = names
        ...
        #    print(x)
            ...
            if "wg" in rfc_list[x]:
                ...
                i = 0
                while i < len(rfc_list[x]["author"]):
                    try:
                        ppl_wg[rfc_list[x]["author"][i]].add(rfc_list[x]["wg"])
                    except KeyError:
                        ppl_wg[rfc_list[x]["author"][i]] = set()
                        ppl_wg[rfc_list[x]["author"][i]].add(rfc_list[x]["wg"])


                    #print(names[i])
                #    print(ppl_wg[names[i]])
